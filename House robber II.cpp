...
public:
    ...
    int solver(vector<int>& v, vector<vector<int>>& dp, int i, int j){
        
        if(i > j) return 0;
        
        if(dp[i][j] != -1) return dp[i][j];
        
        int ans = max(solver(v, dp, i, j-1), solver(v, dp, i, j-2)+v[j]);
    ...
    int rob(vector<int>& v) {
        ...
        int ans = max(solver(v, dp, 0, n-2), solver(v, dp, 1, n-1));
